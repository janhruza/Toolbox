...
public class ColorScheme
{
    ...
    ///     color codes. The value should be a valid ANSI escape sequence recognized by the target terminal.
    /// </remarks>
    ...
    /// </summary>
    /// <param name="colors">
    ///     An array of six strings representing the accent colors. Each element corresponds to Accent1 through Accent6,
    ///     respectively. If the array does not contain exactly six elements, a default ColorScheme is returned.
    /// </param>
    /// <returns>
    ///     A ColorScheme initialized with the specified accent colors if the array contains six elements; otherwise, a
    ///     default ColorScheme instance.
    /// </returns>
    public static ColorScheme FromArray(string[] colors)
    {
        if (colors.Length != 6) return new ColorScheme();
